...
namespace SeeSharpTools.JXI.SignalProcessing.GeneralSpectrum1
{
    ...
    /// </summary>
    public class GeneralSpectrumTask1
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        public GeneralSpectrumTask1(InputDataType1 dataType, double sampleRate)
        {

            //modeChangeFlag = false;
            SpectralInfomation = new SpectralInformation1();
            Unit = new SpectrumUnitInfo1();
            Unit.Impedance = 50;
            _adjustedSampleRate = 10;
            WindowPara = 0;
            // Committed = false;
            InputDataType1 = dataType;
        ...
        public GeneralSpectrumTask1()
        {
            ...
            WindowPara = 0;
           // Committed = false;
            InputDataType1 = InputDataType1.Real;
            _amplitudeAverageAux = new SpectrumAverageAux();
            Average = new SpectrumAverage(this);
            _dftiDescMgr = DFTIDescMgr.GetInstance();
            Output = new SpectrumOutput(this);
        ...
        /// </summary>
        public InputDataType1 InputDataType1 { get; set; }
        ...
        public double SampleRate
        {
            ...
            set
            {
                ...
                var isRealType = InputDataType1.Real == InputDataType1;
        ...
        /// </summary>
        public SpectrumUnitInfo1 Unit { get; }
        ...
        /// </summary>
        public SpectralInformation1 SpectralInfomation { get; }
        #endregion
        ...
        public void GetSpectrum(double[] data, ref double[] spectrum)
        {
            ...
            #endregion 
            if (InputDataType1 == InputDataType1.Complex)
            {
                throw new JXIParamException("Can not call this method when InputDataType1 is Complex.");
            }
            ...
            CBLASNative1.cblas_dcopy(Math.Min(SpectralInfomation.FFTSize, data.Length), data, 1, tempData, 1);
            //}
            ...
            _ENBW = enbw;
            CBLASNative1.cblas_dscal(windowData.Length, 1 / cg, windowData, 1); //窗系数归一化
            VMLNative1.vdMul(windowData.Length, windowData, tempData, tempData);//data通过窗函数
            BasicFFT.RealFFT(tempData, ref xOutCTmp);//实数信号的FFT计算
            SpectralInfomation.FFTCount++;
            VMLNative1.vzAbs(Output.NumberOfLines, xOutCTmp, spectrum);//计算向量元素的绝对值

            double scale = Sqrt2 / SpectralInfomation.FFTSize; //双边到单边有一个二倍关系,输出为Vrms(有效电压)要除以根号2

            //fftcnt次的频谱做平均
            CBLASNative1.cblas_dscal(Output.NumberOfLines, scale, spectrum, 1);//用标量计算向量的乘积
            ...
            Spectrum.UnitConversion(spectrum, SpectralInfomation.FreqDelta, SpectrumType.Amplitude,
                                    unitSettings, enbw);
            if (Average.Mode != SpectrumAverageMode1.NoAveraging)
            {
        ...
        public void GetSpectrum(Complex[] data, ref double[] spectrum)
        {
            ...
            _averagedSpectrum = new double[Output.NumberOfLines];
            #endregion 
            if (InputDataType1 == InputDataType1.Real)
            {
                throw new JXIParamException("Can not call this method when InputDataType1 is Real.");
            }
            if (spectrum.Length < Output.NumberOfLines)
            {
                throw new JXIParamException("spectrum buffer overflow.");
            ...
            //CBLASNative1.cblas_zcopy(data.Length, data, 1, tempData, 1);
            ...
            CBLASNative1.cblas_dcopy(Math.Min(SpectralInfomation.FFTSize, data.Length) * 2, inputPtr, 1, dataPtr, 1);
            gchInput.Free();
            //}
            ...
            _ENBW = enbw;
            CBLASNative1.cblas_dscal(windowData.Length, 1 / cg, windowData, 1); //窗系数归一化
            CBLASNative1.cblas_dcopy(windowData.Length, windowData, 1, windowPtr, 2);
            CBLASNative1.cblas_dcopy(windowData.Length, windowData, 1, windowPtr + sizeof(double), 2);//实部、虚部都要加窗
            VMLNative1.vdMul(windowData.Length * 2, windowDataC, dataPtr, dataPtr);
            gchWindow.Free();
            gchData.Free();

            BasicFFT.ComplexFFT(tempData, ref xOutCTmp);
            SpectralInfomation.FFTCount++;
            VMLNative1.vzAbs(Output.NumberOfLines, xOutCTmp, spectrum);

            double scale = 1.0 / SpectralInfomation.FFTSize; //复数输出为Vrms，不需要除以根号2

            //fftcnt次的频谱做平均
            CBLASNative1.cblas_dscal(Output.NumberOfLines, scale, spectrum, 1);
            ...
            //CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2 + 1, spectrum, 1, tempSpec, 1);
            ...
            if (Output.NumberOfLines % 2 == 0)
            {
                ...
                CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2 , spectrum, 1, tempSpec, 1);

                CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2 ,
            ...
            else
            {
                ...
                CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2 + 1,
                                        tempSpec, 1, specPtr + sizeof(double) * (Output.NumberOfLines / 2), 1);
            }
            gchSpec.Free();
            if (Average.Mode != SpectrumAverageMode1.NoAveraging)
        ...
        public void GetSpectrum(double[] data, ref double[] amplitude, ref double[] phase)
        {
            ...
            //    throw new JXIDSPInnerException("Commit First.");
            //}
            if (Unit.Type != SpectrumOutputUnit1.V)
            {
            ...
            _ENBW = enbw;
            CBLASNative1.cblas_dscal(windowData.Length, 1 / cg, windowData, 1); //窗系数归一化
            VMLNative1.vdMul(windowData.Length, windowData, tempData, tempData);
            BasicFFT.RealFFT(tempData, ref xOutCTmp);
            SpectralInfomation.FFTCount++;
            VMLNative1.vzAbs(Output.NumberOfLines, xOutCTmp, amplitude);
            ...
            CBLASNative1.cblas_dcopy(Output.NumberOfLines, ptr, 2, xOutRe, 1);
            CBLASNative1.cblas_dcopy(Output.NumberOfLines, ptr + sizeof(double), 2, xOutIm, 1);
            VMLNative1.vdAtan2(Output.NumberOfLines, xOutIm, xOutRe,  phase); //计算相位
            gc.Free();

            double scale = Sqrt2 / SpectralInfomation.FFTSize; //双边到单边有一个二倍关系,输出为Vrms要除以根号2

            //fftcnt次的频谱做平均
            CBLASNative1.cblas_dscal(Output.NumberOfLines, scale, amplitude, 1);
        ...
        public void GetSpectrum(Complex[] data, ref double[] amplitude, ref double[] phase)
        {
            ...
            CBLASNative1.cblas_dscal(windowData.Length, 1 / cg, windowData, 1); //窗系数归一化
            CBLASNative1.cblas_dcopy(windowData.Length, windowData, 1, windowPtr, 2);
            CBLASNative1.cblas_dcopy(windowData.Length, windowData, 1, windowPtr + sizeof(double), 2);
            VMLNative1.vdMul(windowData.Length * 2, windowDataC, dataPtr, dataPtr);
            gchWindow.Free();
            gchData.Free();
            ...
            //CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2 + 1, amplitude, 1, tempSpec, 1);
            ...
            if (Output.NumberOfLines % 2 == 0)
            {
                ...
                CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2, amplitude, 1, tempSpec, 1);
            ...
            //CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2 + 1, phase, 1, tempPhase, 1);
            ...
            if (Output.NumberOfLines % 2 == 0)
            {
                ...
                CBLASNative1.cblas_dcopy(Output.NumberOfLines / 2, phase, 1, tempPhase, 1);
        ...
        /// </returns>
        public Peak1 FindPeak(double[] spectrum)
        {
            if (spectrum.Length == 0)
            {
                throw new JXIParamException("输入数组不能为空");
            }
            Peak1 spectrumPeak = new Peak1();//新建一个Peak对象
        ...
        /// <para>Chinese Simplified:输入频谱的最大值所对应的幅值、检索频率、加权频率</para>
        /// </returns>
        public Peak1 FindPeak(double[] spectrum, double freqStart, double freqStop)
        {
            if (spectrum.Length == 0)
            {
                throw new JXIParamException("输入数组不能为空");
        ...
        /// </returns>
        public Peak1 [] FindPeak(double[] spectrum,double threshold)
        {
            if (spectrum.Length==0)//判断数组是否为空
            {
                return null;
            }
            List<Peak1> spectrumPeak=new List<Peak1>();//不知道具体有几个峰值，所以定义链表
            Peak1 spectrumPeakOverthreshold = new Peak1();//新建一个Peak对象
        ...
        /// </returns>
        public Peak1[] FindPeak(double[] spectrum, double threshold, double freqStart, double freqStop)
        {
            if (spectrum.Length == 0)//判断数组是否为空
            {
                return null;
        ...
        public double MeasurePowerInBand(double[] spectrum, double centerFreq, double bandwidth)
        {
            ...
            //判断参数的合理性
            if (InputDataType1== InputDataType1.Complex &&(centerFreq - bandwidth / 2 < -SampleRate / 2 || centerFreq + bandwidth / 2 > SampleRate / 2 || bandwidth<=0)||
                InputDataType1 == InputDataType1.Real && (centerFreq - bandwidth / 2 < 0 || centerFreq + bandwidth / 2 > SampleRate / 2 || bandwidth <= 0)
                )
            {
            ...
            if (!Unit.IsPSD)
            {
                ...
                CBLASNative1.cblas_dscal(spec.Length, scale, spec, 1);
            }
            //计算带内功率
