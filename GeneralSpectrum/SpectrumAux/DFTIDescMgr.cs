    ...
    internal class DFTIDescMgr:IDisposable
    {
            ...
            lock (_dftDescDict)
            {
                ...
                switch (type)
                {
                    case DFTType.Double1DRealInComplexOut:
                        precision = DFTI1.DOUBLE;
                        forwardDomain = DFTI1.REAL;
                        dimension = 1;
                        if (0 != (ret = DFTI1.DftiCreateDescriptor(ref desc, precision, forwardDomain,
                                                        dimension, length)))
                        {
                            throw new JXIInnerException("DftiCreateDescriptor Failed, error code = " + ret);
                        }

                        if (0 != (ret = DFTI1.DftiSetValue(desc, DFTI1.PACKED_FORMAT, DFTI1.CCS_FORMAT)))
                        {
        ...
        public void Dispose()
        {
            ...
            foreach (var desc in _dftDescDict)
            {
                ...
                if (0 != (ret = DFTI1.DftiFreeDescriptor(ref descs)))
                {
