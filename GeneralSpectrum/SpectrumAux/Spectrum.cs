...
namespace SeeSharpTools.JXI.SignalProcessing.GeneralSpectrum1
{
    ...
    internal static class Spectrum
    {
        ...
        public static void PowerSpectrum(double[] x, double samplingRate, ref double[] spectrum, out double df,
            SpectrumOutputUnit1 unit = SpectrumOutputUnit1.V2, WindowType windowType = WindowType.Hanning,
...

        {
            ...
            CBLASNative1.cblas_dscal(spectralLines, scale, spectrum, 1);
        ...
        public static void PeakSpectrumAnalysis(double[] timewaveform, double dt, out double peakFreq, out double peakAmp)
        {
            ...
            double df;
            var spectUnit = SpectrumOutputUnit1.V2; //this V^2 unit relates to power in band calculation, don't change
            ...
            UnitConvSetting unitSetting, double equivalentNoiseBw)
        {
            ...
            //VMLNative1.vdSqr(N, spectrum, spectrum);
            ...
            if (unitSetting.PeakScaling == PeakScaling.Peak) //峰峰值要乘以2
            {
                ...
                CBLASNative1.cblas_dscal(N, scale, spectrum, 1);
            ...
            {
                case SpectrumOutputUnit1.V:
                    {
                        if (SpectrumType.Power == spectrumType)
                        {
                            VMLNative1.vdSqrt(N, spectrum, spectrum);
                        }
                ...
                case SpectrumOutputUnit1.dBV:
                    {
                        ...
                        //lg
                        VMLNative1.vdLog10(N, spectrum, spectrum);
                        scale = 20;
                        //20*lg
                        CBLASNative1.cblas_dscal(N, scale, spectrum, 1);
                ...
                case SpectrumOutputUnit1.dBmV:
                    {
                        ...
                        CBLASNative1.cblas_dscal(N, 1e3, spectrum, 1);  //V To mV                
                        VMLNative1.vdLog10(N, spectrum, spectrum);    //To Lg
                        scale = 20;

                        CBLASNative1.cblas_dscal(N, scale, spectrum, 1); //To 20*Lg
                        break;
                    }
                case SpectrumOutputUnit1.dBuV:
                    {
                        if (SpectrumType.Power == spectrumType)
                        {
                            VMLNative1.vdSqrt(N, spectrum, spectrum);
                        ...
                        CBLASNative1.cblas_dscal(N, 1e6, spectrum, 1); //V To uV                
                        VMLNative1.vdLog10(N, spectrum, spectrum);//To Lg
                        scale = 20;
                        CBLASNative1.cblas_dscal(N, scale, spectrum, 1);//To 20*Lg
                        break;
                    }
                case SpectrumOutputUnit1.V2:
                    {
                ...
                case SpectrumOutputUnit1.W:
                case SpectrumOutputUnit1.dBW:
                case SpectrumOutputUnit1.dBm:
                    {
                        if (SpectrumType.Amplitude == spectrumType)
                        {
                            //计算V2，平方
                        ...
                        //VMLNative1.vdSqr(N, spectrum, spectrum);         //V^2
                        CBLASNative1.cblas_dscal(N, scale, spectrum, 1); //W = V^2/R

                        if (unitSetting.Unit == SpectrumOutputUnit1.dBW)   //dBW = 20lgW
                        {
                            //lg
                            VMLNative1.vdLog10(N, spectrum, spectrum);
                            scale = 10;
                        ...
                        else if (unitSetting.Unit == SpectrumOutputUnit1.dBm) // dBm = 10lg(W/1mW)
                        {
                            CBLASNative1.cblas_dscal(N, 1e3, spectrum, 1); // W/1mW
                                                              //lg
                            VMLNative1.vdLog10(N, spectrum, spectrum);
                            scale = 10;
                            //10*lg
        ...
        internal static void UnitConversion(ref double[] spectrum, SpectrumOutputUnit1 unit)
        {
          switch (unit)
            {
                case SpectrumOutputUnit1.dBW:
                case SpectrumOutputUnit1.dBm:
                    {
                        double scale = 1.0/10;
                        CBLASNative1.cblas_dscal(spectrum.Length, scale, spectrum, 1);
        ...
        /// <param name="spectrum"></param>
        /// <param name="unit"></param>
        internal static void UnitConversionToV2(ref double[] spectrum, SpectrumUnitInfo1 unit)
        {
            ...
            switch (unit.Type)
            {
                ...
                case SpectrumOutputUnit1.V:
                    ...
                    break;
                case SpectrumOutputUnit1.W:
                    CBLASNative1.cblas_dscal(spectrum.Length, unit.Impedance, spectrum, 1);
                    break;
                case SpectrumOutputUnit1.dBW:
                    scale = 1.0 / 10;
                    CBLASNative1.cblas_dscal(spectrum.Length, scale, spectrum, 1);
        ...
        /// <param name="unit"></param>
        /// <returns></returns>
        internal static double UnitConversionV2ToOther( double data, SpectrumUnitInfo1 unit)
        {
            double PowerInBand = 0;
                                     ...
                                     double[] xOut, ref SpectralInfo spectralInfo)
        {
            ...
            Window.Window.GetWindow(windowType, ref windowData, out cg, out enbw);
            CBLASNative1.cblas_dscal(windowsize, 1 / cg, windowData, 1); //窗系数归一化
            CBLASNative1.cblas_dscal(xOut.Length, 0, xOut, 1); //将xOut清零
            ...
            for (int i = 0; i < fftcnt; i++)
            {
                ...
                VMLNative1.vdMul(windowsize, windowData, xInPtr + i * fftsize * sizeof(double), xInTmp);
                BasicFFT.RealFFT(xInTmp, ref xOutCTmp);

                //计算FFT结果复数的模,复用x_in做中间存储
                VMLNative1.vzAbs(fftsize / 2 + 1, xOutCTmp, xInTmp);

                //每次计算结果累加起来
                VMLNative1.vdAdd(spectralLines, xInTmp, xOut, xOut);
            }

            scale = 2 * (1.0 / fftcnt) / Sqrt2; //双边到单边有一个二倍关系,输出为Vrms要除以根号2

            //fftcnt次的频谱做平均
            CBLASNative1.cblas_dscal(spectralLines, scale, xOut, 1);
        ...
        internal static void AdvanceComplexFFT(double[] waveform, WindowType windowType, ref Complex[] spectrum)
        {
            ...
            try
            {
                ...
                Window.Window.GetWindow(windowType, ref windowData, out cg, out enbw);
                CBLASNative1.cblas_dscal(windowsize, 1 / cg, windowData, 1); //窗系数归一化
                CBLASNative1.cblas_dscal(spectrum.Length, 0, xOutPtr, 1); //将xOut清零
                /*TIME_DOMAIN_WINDOWS(windowType, x_in, &CG, &ENBW, windowsize);*//*(double*)(xIn + i * windowsize)*/
                VMLNative1.vdMul(windowsize, windowData, xInPtr, xInTmp);
        ...
        internal static void AdvanceComplexFFT(Complex[] waveform, WindowType windowType, ref Complex[] spectrum)
        {
            ...
            CBLASNative1.cblas_dcopy( waveform.Length* 2, xInPtr, 1, dataPtr, 1);
            try
            {
                //生成窗函数的数据
                windowData = new double[windowsize];
                ...
                CBLASNative1.cblas_dscal(spectrum.Length, 0, xOutPtr, 1); //将xOut清零
                CBLASNative1.cblas_dcopy(windowData.Length, windowData, 1, windowPtr, 2);//复制real窗函数变成complex的窗函数
                CBLASNative1.cblas_dcopy(windowData.Length, windowData, 1, windowPtr + sizeof(double), 2);
                VMLNative1.vdMul(windowData.Length * 2, windowDataC, dataPtr, dataPtr);//将信号通过窗函数
