...
namespace SeeSharpTools.JXI.SignalProcessing.GeneralSpectrum1
{
    internal class SpectrumAverageAux
    {
        ...
        {
            _averageCount = 0;
            _averagingMode = SpectrumAverageMode1.NoAveraging;
            _averagingSize = 10;
            _spectrumList = new List<double[]>();
            _vectorList = new List<Complex[]>();
            _weightingType = SpectrumWeightingType1.LinearMoving;
        ...
        /// 平均模式
        /// </summary>
        private SpectrumAverageMode1 _averagingMode;
        ...
        /// 平均的加权方式
        /// </summary>
        private SpectrumWeightingType1 _weightingType;
        ...
        /// </summary>
        public SpectrumAverageMode1 Mode
        {
            get
            {
        ...
        /// 加权类型
        /// </summary>
        public SpectrumWeightingType1 WeightingType
        {
        ...
        {
            if (_averagingMode == SpectrumAverageMode1.VectorAveraging)
            {
            ...
            if (_averagedSpectrum == null || _averagedSpectrum.Length != newSpectrum.Length)
            {
                ...
                CBLASNative1.cblas_dcopy(newSpectrum.Length, newSpectrum, 1, _averagedSpectrum, 1);
                _averageCount++;
                retFlag = true;
            }
            if (Mode == SpectrumAverageMode1.RMSAveraging)
            {
                var newSpectrumToAdd = new double[newSpectrum.Length];
                _spectrumList.Add(newSpectrumToAdd);
                CBLASNative1.cblas_dcopy(newSpectrum.Length, newSpectrum, 1, newSpectrumToAdd, 1);
            }
            if (retFlag)
            {
                return;
            }
            if (Mode != SpectrumAverageMode1.NoAveraging && _averageCount == 0)
            {
                CBLASNative1.cblas_dcopy(newSpectrum.Length, newSpectrum, 1, _averagedSpectrum, 1);
                _averageCount++;
                return;
            ...
            {
                case SpectrumAverageMode1.NoAveraging:
                    {
                        //Buffer.BlockCopy(newSpectrum, 0, _averagedSpectrum, 0, newSpectrum.Length * sizeof(double));
                        break;
                    }
                case SpectrumAverageMode1.RMSAveraging:
                    {
                        if (_weightingType == SpectrumWeightingType1.LinearMoving)
                        {
                            if (_averageCount < _averagingSize)
                            {
                                //(averagedSpectrum * _averageCount + newSpectrum) / (_averageCount + 1);
                                CBLASNative1.cblas_dscal(_averagedSpectrum.Length, _averageCount, _averagedSpectrum, 1);
                                VMLNative1.vdAdd(_averagedSpectrum.Length, _averagedSpectrum, newSpectrum,
                                    _averagedSpectrum);
                                CBLASNative1.cblas_dscal(_averagedSpectrum.Length, 1.0/(_averageCount + 1),
                                    _averagedSpectrum, 1);
                                _averageCount++;
                            }
                            else
                            {
                                ...
                                CBLASNative1.cblas_dscal(_averagedSpectrum.Length, _averageCount, _averagedSpectrum, 1);
                                VMLNative1.vdSub(_averagedSpectrum.Length, _averagedSpectrum, _spectrumList[0], _averagedSpectrum);
                                VMLNative1.vdAdd(_averagedSpectrum.Length, _averagedSpectrum, newSpectrum, _averagedSpectrum);
                                CBLASNative1.cblas_dscal(_averagedSpectrum.Length, 1.0 / _averageCount,
                                    _averagedSpectrum, 1);
                                _spectrumList.RemoveAt(0); //Remove First
                            }
                        }
                        else if (_weightingType == SpectrumWeightingType1.LinearContinuous)
                        {
                            //(averagedSpectrum * _averageCount + newSpectrum) / (_averageCount + 1);
                            CBLASNative1.cblas_dscal(_averagedSpectrum.Length, _averageCount, _averagedSpectrum, 1);
                            VMLNative1.vdAdd(_averagedSpectrum.Length, _averagedSpectrum, newSpectrum,
                        ...
                        else  //weightingType == SpectrumWeightingType1.Exponential.
                        {
                            throw new NotImplementedException();
                            //double a1, a2 = 0.5;
                ...
                case SpectrumAverageMode1.PeakHold:
                    {
                        for (int i = 0; i < newSpectrum.Length; i++)
                        {
            ...
            switch (_averagingMode)
            {
                ...
                case SpectrumAverageMode1.RMSAveraging:
                {
                    ...
                    if (_weightingType == SpectrumWeightingType1.LinearMoving)
                    {
                        ...
                        {
                            //(averagedSpectrum * _averageCount + newSpectrum) / (_averageCount + 1);
                            CBLASNative1.cblas_dscal(averagedSpectrum.Length, _averageCount*2, averagedSpectrumPtr, 1);
                            VMLNative1.vdAdd(averagedSpectrum.Length*2, averagedSpectrumPtr, newSpectrumPtr,
                                averagedSpectrumPtr);
                            CBLASNative1.cblas_dscal(averagedSpectrum.Length*2, 1.0 / (_averageCount + 1),
                        ...
                        else
                        {
                            ...
                            CBLASNative1.cblas_dscal(averagedSpectrum.Length*2, _averageCount, averagedSpectrumPtr, 1);
                            VMLNative1.vdSub(averagedSpectrum.Length*2, averagedSpectrumPtr, vectorList0Ptr,
                                averagedSpectrumPtr);
                            VMLNative1.vdAdd(averagedSpectrum.Length*2, averagedSpectrumPtr, newSpectrumPtr,
                                averagedSpectrumPtr);
                            CBLASNative1.cblas_dscal(averagedSpectrum.Length*2, 1.0 / (_averageCount + 1), averagedSpectrumPtr, 1);
                    ...
                    else if (_weightingType == SpectrumWeightingType1.LinearContinuous)
                    {
                        //(averagedSpectrum * _averageCount + newSpectrum) / (_averageCount + 1);
                        CBLASNative1.cblas_dscal(averagedSpectrum.Length * 2, _averageCount, averagedSpectrumPtr, 1);
                        VMLNative1.vdAdd(averagedSpectrum.Length * 2, averagedSpectrumPtr, newSpectrumPtr, averagedSpectrumPtr);
                        CBLASNative1.cblas_dscal(averagedSpectrum.Length * 2, 1.0 / (_averageCount + 1), averagedSpectrumPtr, 1);
                ...
                case SpectrumAverageMode1.PeakHold:
                {
                    ...
                    VMLNative1.vzAbs(averagedSpectrum.Length, averagedSpectrum, averagedAmp);
                    VMLNative1.vzAbs(newSpectrum.Length, newSpectrum, newAmp);
                    for (int i = 0; i < newSpectrum.Length; i++)
                    {
