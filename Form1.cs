...
namespace test1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            formsPlot1.Plot.XLabel("频率(MHz)");
            formsPlot1.Plot.YLabel("幅值(dBFS)");
            formsPlot1.Plot.Title("频谱图");

            formsPlot1.Plot.SetAxisLimitsX(0, 500);
            formsPlot1.Plot.SetAxisLimitsY(-180, 0);
            formsPlot1.Plot.SetOuterViewLimits(0, 500, -180, 0);
        }
        ...
        private void unpackdata_dat()
        {
            ...
            ushort tail = G.ReadUInt16();

            byte[] rsv_tail = G.ReadBytes(62);

            if (tail != 0x91eb)
            {
                showLog("pack tail is not found, skip four bytes");
            }
        }
        ...
        public static (double[], double[]) GetFftAsync(double[] inputData, double sampleRate)
        {
            // 补充长度
            int nlen = 2;
            while (nlen < inputData.Length)
            {
                nlen *= 2;
            }
            double[] outputData = new double[nlen];
            inputData.CopyTo(outputData, 0);

            // 计算power freq
            double[] power = FftSharp.Transform.FFTpower(outputData);
            double[] freq = FftSharp.Transform.FFTfreq(sampleRate, power);

            // Hz转MHz
            freq = (from x in freq select x / 1e6).ToArray();

            return (freq, power);
        }

        public static double[] DataProcessAsync(ushort[] rawData)
        {
            // 归一化 -1 1
            double[] output = (from x in rawData select (x - 0x8000) * 1.0 / 0x8000).ToArray();

            // 去直流偏置
            double men = output.Average();
            output = (from x in output select x - men).ToArray();

            // 加窗
            output = new FftSharp.Windows.Hanning().Apply(output);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // 获取链表packs的长度
            int plen = packs.Count;

            int i, j;

            double[] ave = new double[plen];                //ave是平均值

            double[][] unpacked = new double[plen][];

            double[] ENOB = new double[plen], SINAD = new double[plen], SNR = new double[plen], THD = new double[plen];
            ...
            finalENOB = finalENOB / plen;
            finalSINAD = finalSINAD / plen;
            finalSNR = finalSNR / plen;
            finalTHD = finalTHD / plen;
            ...
            */
            ...
            Console.WriteLine(ENOB[0]);
            Console.WriteLine(SINAD[0]);
            Console.WriteLine(SNR[0]);
            Console.WriteLine(-THD[0]);
            ...
            // 频域
            (freq, power) = GetFftAsync(DataProcessAsync(adccode), 1e9);
            //Plot(freq, power, _scottPlot);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            formsPlot1.Plot.AddSignalXY(freq, power, Color.Red);
            //formsPlot1.Plot.AddFill(freq, power);
            formsPlot1.Plot.SaveFig("spectral_diagram.png");
